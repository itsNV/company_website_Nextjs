...
export default function SolutionsAdminPage() {
  ...
  const handlePublish = async () => {
    if (!name || !slug) {
      setMessage({ type: "error", text: "Solution Name and Slug are required to publish." });
      return;
    }
    ...
    try {
      ...
      // 3. Save to Firestore
      const targetId = editingSolutionId || slug;
      await setDoc(doc(db, "solutions", targetId), {
        ...payload,
        createdAt: editingSolutionId ? (solutions.find(s => s.id === editingSolutionId)?.createdAt || new Date().toISOString()) : new Date().toISOString()
      });

      setMessage({ 
        type: "success", 
        text: editingSolutionId ? "Solution configurations saved successfully!" : "New Solution deployed successfully!" 
      });
      ...
      setMessage({ type: "error", text: err.message || "Failed to publish solution catalog updates." });
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <AdminGuard>
      <div className="flex flex-col min-h-screen bg-[#f3f9fc] text-slate-800 font-sans relative z-10">
        ...
        <main className="flex-grow pt-28 pb-16 px-6 max-w-7xl mx-auto w-full z-10">
          <div className="absolute top-0 left-1/4 w-[400px] h-[400px] bg-emerald-200/20 rounded-full blur-[140px] pointer-events-none" />
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border-b border-slate-200 pb-6 mb-10">
            <div>
              ...
              <p className="text-slate-500 text-xs mt-1 uppercase tracking-wider font-semibold">
                Visual Drag & Drop solution configurator
              </p>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white border border-slate-200/60 rounded-3xl p-6 shadow-xl max-w-4xl mx-auto">
                <div className="flex items-center gap-2 border-b border-slate-100 pb-3 mb-4">
                  ...
                  <h2 className="text-sm font-black text-slate-900 uppercase tracking-wider font-outfit">
                    {editingSolutionId ? "Modify Blueprint Details" : "Setup New Solution Track"}
                  </h2>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input
                      type="text"
                      required
                      placeholder="e.g. ERP Enterprise Solutions"
                      value={name}
              ...
              <div className="mt-6">
                <AdminPageBuilder 
                  ...
                  publishLabel={editingSolutionId ? "Save Changes" : "Create Solution"}
                />
              </div>
            </div>
          ) : (
            ...
            <div className="max-w-4xl mx-auto space-y-6">
              ...
              {loadingList ? (
                <div className="flex flex-col items-center justify-center py-20 text-slate-400 gap-3">
                  ...
                  <span className="text-xs uppercase font-bold tracking-wider">Syncing blueprint details...</span>
                </div>
              ) : filteredSolutions.length === 0 ? (
                <div className="bg-white border border-slate-200/60 rounded-3xl p-12 text-center text-slate-500 shadow-md">
                  <p className="text-sm font-semibold">No solutions found matching current search.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-4">
                  {filteredSolutions.map((sol) => (
                    <div key={sol.id} className="bg-white border border-slate-200/60 rounded-3xl p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
